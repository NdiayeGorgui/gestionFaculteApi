...
@Component
@Aspect
public class LogAspectUpdate {
	Logger logger=Logger.getLogger(LogAspectUpdate.class.getName());
	
    public LogAspectUpdate() throws IOException {
        logger.addHandler(new FileHandler("myLogUpdate.xml"));  //creer un fic log
        logger.setUseParentHandlers(false);  //pour lui dire de ne plus logger dans la console
    }
    
    @Pointcut(value="execution(* com.myproject.faculte.*.*..update*(..))")

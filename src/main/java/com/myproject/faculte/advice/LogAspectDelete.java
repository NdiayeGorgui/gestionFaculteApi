...
@Component
@Aspect
public class LogAspectDelete {
	Logger logger=Logger.getLogger(LogAspectDelete.class.getName());
	

    public LogAspectDelete() throws IOException {
        logger.addHandler(new FileHandler("myLogDelete.xml"));  //creer un fic log
        logger.setUseParentHandlers(false);  //pour lui dire de ne plus logger dans la console
    }
    
    @Pointcut(value="execution(* com.myproject.faculte.*.*..delete*(..))")
